...
class SSF_Env(gym.Env):
    ...
    def __init__(self, gametype="explode", scale=.213, ls=3):
        self._seed()
    ...
    def _reset(self):
        ...
        if self.videofile and not self.video:
            self.video = cv2.VideoWriter("%s.avi" % self.videofile, cv2.VideoWriter_fourcc(*"H264"), self.metadata['video.frames_per_second'], (self.w,self.h))
    ...
    def _step(self, action):
        ...
        if self.videofile:
            ...
            if done:
                cv2.imwrite("%s.png" % self.videofile, img)
                self.video.release()
